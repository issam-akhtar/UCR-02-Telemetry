...
const char* mqttUser = "test_user"; // Replace with your username
const char* mqttPassword = "test_password"; // Replace with your password

WiFiClient espClient;
PubSubClient client(espClient);

// Simulated CAN packet data and timestamps (replace with parsed CSV data)
struct CanPacket {
  float timestamp;
  const char* data;
};
...
int packetIndex = 0;
unsigned long lastPacketTime = 0; // Time of last packet sent
...
void loop() {
  ...
  if (packetIndex < (sizeof(canData) / sizeof(canData[0]))) {
    float timeDiff = 0;
    if (packetIndex > 0) {
      // Calculate expected delay based on timestamps
    ...
    if (currentMillis - lastPacketTime >= timeDiff) {
      // Measure delay before sending
      unsigned long waitTime = currentMillis - lastPacketTime;

      // Send the packet
      sendPacket();

      // Output the delay measurements
      Serial.print("Expected Delay: ");
      Serial.print(timeDiff);
      Serial.print(" ms, Actual Delay: ");
    ...
    packetIndex = 0;  // Restart for continuous testing
  }
}

void sendPacket() {
  unsigned long startTime = millis(); // Start timing the send operation

  String newPacket = canData[packetIndex].data;
  Serial.print("Sending CAN packet: ");
  Serial.println(newPacket);
  client.publish("car/raw_sensors", newPacket.c_str());

  unsigned long sendDuration = millis() - startTime;
  Serial.print("Packet send duration: ");
