...
static void twai_control_task(void *arg)
{
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++)
    {
        ...
        ESP_LOGI(EXAMPLE_TAG, "send queue");
        xQueueSend(rx_task_queue, &rx_action, portMAX_DELAY);
        ESP_LOGI(EXAMPLE_TAG, "Take sem");
        xSemaphoreTake(ctrl_task_sem, portMAX_DELAY);
        ESP_LOGI(EXAMPLE_TAG, "Listening for Pings");
        ...
        xQueueSend(rx_task_queue, &rx_action, portMAX_DELAY);
        xSemaphoreTake(ctrl_task_sem, portMAX_DELAY);
        ESP_LOGI(EXAMPLE_TAG, "Listening for Start");
...
void app_main(void)
{
    // Delay to start transmission from SLCAN
    for (int i = 3; i > 0; i--)
    {
    ...
    // NOt gonna try any core for now
    xTaskCreatePinnedToCore(twai_receive_task, "TWAI_rx", 4096, NULL, RX_TASK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(twai_transmit_task, "TWAI_tx", 4096, NULL, TX_TASK_PRIO, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(twai_control_task, "TWAI_ctrl", 4096, NULL, CTRL_TSK_PRIO, NULL, tskNO_AFFINITY);

    ESP_ERROR_CHECK(twai_driver_install(&g_config, &t_config, &f_config)); // Install TWAI driver, trigger tasks to start
    ESP_LOGI(EXAMPLE_TAG, "Driver installed Successfully");
    // Start Control task
    xSemaphoreGive(ctrl_task_sem);
