...
const valueToColor = (value) => {
  if (value >= 2.5 && value <= 2.7) return new THREE.Color(1, 0, 0);
  if (value > 2.7 && value <= 3.1) return new THREE.Color(1, 0.5, 0);
  if (value > 3.1 && value <= 3.4) return new THREE.Color(1, 1, 0);
  if (value > 3.4 && value <= 3.8) return new THREE.Color(0, 1, 0);
  return new THREE.Color(0.2, 0.2, 0.2);
};

const Model = ({ cellValues, setHoveredCell }) => {
  const { scene, nodes } = useGLTF("/goodvoltagemodel.glb");

  // Create mesh name → cell name lookup map
  const meshToCellMap = {};
  ...
  // Update colors based on voltage
  useEffect(() => {
  ...
  return (
    <primitive
      ...
      onPointerOver={(event) => {
        ...
        const cellName = meshToCellMap[meshName];

        console.log("Hovered Mesh:", meshName);
        if (cellName) {
          console.log("Mapped To Cell:", cellName);
...
const CellVoltageScene = () => {
  ...
  // Simulate incoming voltage values
  useEffect(() => {
    const interval = setInterval(() => {
      setCellValues((prev) => {
        const newValues = { ...prev };
        Object.keys(newValues).forEach((cell) => {
          newValues[cell] = +(2.5 + Math.random() * (3.8 - 2.5)).toFixed(2); //min 2.5 max 3.8
        });
        return newValues;
      });
    }, 1000);
  ...
  return (
    <div style={{ height: "100vh", width: "100vw", position: "relative" }}>
      {hoveredCell && (
        ...
        >
          <div><strong>{hoveredCell.name}</strong></div>
          <div>Voltage: {hoveredCell.voltage.toFixed(2)} V</div>
        </div>
      )}
      ...
      <Canvas camera={{ position: [0.8, 0.2, 0], fov: 45 }}>
        ...
        <Model cellValues={cellValues} setHoveredCell={setHoveredCell} />
        <OrbitControls />
      </Canvas>
    </div>
  );
